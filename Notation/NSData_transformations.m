...
- (NSMutableData *)uncompressedData {
	...
	if ([self isCompressedFormat]) {
		...
		NS_DURING newData = [NSMutableData dataWithLength:originalSize];
				NS_HANDLER if ([[localException name] isEqualToString:NSInvalidArgumentException]) {
...
- (NSMutableString *)newStringUsingBOMReturningEncoding:(NSStringEncoding *)encoding {
	...
	if (foundBOM) {
		...
		string = [[NSMutableString alloc] initWithCharactersNoCopy: (unichar *)u length: len / 2 freeWhenDone: YES];
